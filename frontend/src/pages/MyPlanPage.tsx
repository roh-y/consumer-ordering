    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center py-16 bg-white rounded-xl shadow-sm">
          ...
          <p className="text-text-secondary mb-6">Browse our plans to find the perfect fit for you.</p>
          <button
            onClick={() => navigate('/plans')}
            className="bg-primary hover:bg-primary-hover text-white px-8 min-h-[48px] rounded-full font-semibold transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
          >
            Browse Plans
          </button>
        </div>
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-text-primary">{currentPlan.name}</h2>
            <p className="text-sm text-text-secondary mt-1">{currentPlan.description}</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs px-3 py-1 rounded-full font-semibold bg-green-50 text-success">
              Active
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-bg-secondary rounded-xl p-4">
            <div className="text-xs font-medium text-text-secondary mb-1">Monthly</div>
            <div className="text-2xl font-bold text-text-primary">
        ...
        <div className="flex gap-3">
          ...
          {activeOrder && (
            <button
              ...
              className="flex-1 text-error text-sm font-medium min-h-[44px] rounded-full hover:bg-red-50 transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
            >
              Cancel Plan
            </button>
          )}
          ...
          <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-error text-white min-h-[44px] rounded-full text-sm font-semibold disabled:opacity-50 transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
              >
                {cancelMutation.isPending ? 'Cancelling...' : 'Yes, Cancel Plan'}
              </button>
              <button
                onClick={() => setShowConfirm(false)}
                className="flex-1 border border-border-default text-text-primary min-h-[44px] rounded-full text-sm font-semibold hover:bg-bg-secondary transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
              >
                Keep Plan
              </button>
            </div>
      ...
      <button
        ...
        className="mt-4 text-text-secondary hover:text-text-primary text-sm min-h-[44px] focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded-lg"
      >
        View order history →
      </button>
    </div>
