...
function OrderCard({ order, muted }: { order: EnrichedOrder; muted?: boolean }) {
  ...
  return (
    <button
      ...
      className={`w-full rounded-xl shadow-sm p-5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 text-left focus-visible:ring-2 focus-visible:ring-[--color-primary] focus-visible:ring-offset-2 min-h-[44px] ${
        muted ? 'bg-[--color-bg-secondary] opacity-70' : 'bg-white'
      }`}
    >
      <div className="flex items-center justify-between mb-2">
        <h3 className="font-semibold text-[--color-text-primary]">{order.planName}</h3>
        <span
      ...
      <div className="flex items-center justify-between text-sm text-[--color-text-secondary]">
        <div className="flex gap-3">
          <span className="font-medium text-[--color-text-primary]">${order.pricePerMonth}/mo</span>
          {order.plan && (
            <>
...
export default function OrdersPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-[--color-text-primary] mb-1">My Orders</h1>
      <p className="text-[--color-text-secondary] text-sm mb-6">View your plan subscriptions</p>

      {!hasAny ? (
        <div className="text-center py-16 bg-white rounded-xl shadow-sm">
          <p className="text-[--color-text-secondary] mb-4">No orders yet</p>
          <button
        ...
        <div className="space-y-8">
          {hasActiveOrPending && (
            <section>
              <h2 className="text-base font-semibold text-[--color-text-primary] mb-3">Active & Pending</h2>
          ...
          {cancelledOrders.length > 0 && (
            <section>
              <h2 className="text-base font-semibold text-[--color-text-tertiary] mb-3">Past Plans</h2>
              <div className="space-y-3">
